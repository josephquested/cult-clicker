...
function indoctrinateFollowerButtonClicked() {
    if (stuff.faith >= indoctrinateCost) {
        ...
        indoctrinateCost = Math.round(indoctrinateCost *= indoctrinateCostMultiplier)
...
function createFollowerHTML(follower) {
    ...
    // housing button binding
    document.getElementById(`housing-button-${follower.id}`).onclick = bindHousingSelect
...
function bindHousingSelect(e) {
    ...
    let prevStatus = follower.homeless

    if (follower.homeless) {
        if (stuff.housing >= 1) 
            makeFollowerHomeless(follower, false)
        else
            print("no available housing!")
    }
    else 
